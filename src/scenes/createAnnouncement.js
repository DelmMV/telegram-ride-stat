...
createAnnouncementScene.on('text', async (ctx) => {
    ...
    if (!ctx.scene.state.step) {
        ...
        await updateMessage(
            'Дата: ' + ctx.scene.state.announcement.date + '\n' +
            'Название: ' + text + '\n\n' +
            'Введите место сбора:',
            Markup.keyboard([['❌ Отмена']]).resize()
        )
        ...
        await updateMessage(
            'Дата: ' + ctx.scene.state.announcement.date + '\n' +
            'Название: ' + ctx.scene.state.announcement.name + '\n' +
            'Место сбора: ' + text + '\n\n' +
        ...
        await updateMessage(
            'Дата: ' + ctx.scene.state.announcement.date + '\n' +
            'Название: ' + ctx.scene.state.announcement.name + '\n' +
            'Место сбора: ' + ctx.scene.state.announcement.meetingPlace + '\n' +
            'Время старта: ' + text + '\n\n' +
        ...
        await updateMessage(
            ...
            'Название: ' + ctx.scene.state.announcement.name + '\n' +
            'Место сбора: ' + ctx.scene.state.announcement.meetingPlace + '\n' +
            'Время старта: ' + ctx.scene.state.announcement.startTime + '\n' +
            'Длина маршрута: ' + text + ' км\n\n' +
        ...
        await updateMessage(
            ...
            'Место сбора: ' + ctx.scene.state.announcement.meetingPlace + '\n' +
            'Время старта: ' + ctx.scene.state.announcement.startTime + '\n' +
            'Длина маршрута: ' + ctx.scene.state.announcement.routeDistance + ' км\n' +
            'Трек: ' + (text === '-' ? 'нет' : text) + '\n\n' +
        ...
        await updateMessage(
            ...
            'Время старта: ' + ctx.scene.state.announcement.startTime + '\n' +
            'Длина маршрута: ' + ctx.scene.state.announcement.routeDistance + ' км\n' +
            'Трек: ' + (ctx.scene.state.announcement.routeLink || 'нет') + '\n' +
            'Зарядки: ' + text + '\n\n' +
        ...
        if (text === '✅ Готово') {
            if (!ctx.scene.state.announcement.roadTypes || ctx.scene.state.announcement.roadTypes.length === 0) {
            ...
            await updateMessage(
                ...
                'Длина маршрута: ' + ctx.scene.state.announcement.routeDistance + ' км\n' +
                'Трек: ' + (ctx.scene.state.announcement.routeLink || 'нет') + '\n' +
                'Зарядки: ' + ctx.scene.state.announcement.charges + '\n' +
                'Типы дорог: ' + selectedTypes + '\n\n' +
            ...
            const roadType = text
            const existingType = ctx.scene.state.announcement.roadTypes.find(t => t.name === roadType)
            if (existingType) {
                existingType.selected = !existingType.selected
            } else {
                ctx.scene.state.announcement.roadTypes.push({ name: roadType, selected: true })
            }
            const selectedTypes = ctx.scene.state.announcement.roadTypes
                .filter(t => t.selected)
                .map(t => t.name)
        ...
        if (!speedRegex.test(text)) {
            await ctx.reply('Неверный формат скорости. Используйте формат "от X до Y"')
            return
        }
        ctx.scene.state.announcement.speed = text.replace('от ', '').replace(' до ', '-')
        ...
        await updateMessage(
            ...
            'Трек: ' + (ctx.scene.state.announcement.routeLink || 'нет') + '\n' +
            'Зарядки: ' + ctx.scene.state.announcement.charges + '\n' +
            'Типы дорог: ' + ctx.scene.state.announcement.roadTypes.filter(t => t.selected).map(t => t.name).join(', ') + '\n' +
            'Скорость: ' + text + '\n\n' +
        ...
        await updateMessage(
            ...
            'Зарядки: ' + ctx.scene.state.announcement.charges + '\n' +
            'Типы дорог: ' + ctx.scene.state.announcement.roadTypes.filter(t => t.selected).map(t => t.name).join(', ') + '\n' +
            'Скорость: от ' + ctx.scene.state.announcement.speed.split('-')[0] + ' до ' + ctx.scene.state.announcement.speed.split('-')[1] + '\n' +
            'Описание: ' + text + '\n\n' +
        ...
        if (text !== '-') {
            const additionalOrganizers = text.split(',').map(org => org.trim()).filter(org => org)
        ...
        await updateMessage(
            ...
            'Типы дорог: ' + ctx.scene.state.announcement.roadTypes.filter(t => t.selected).map(t => t.name).join(', ') + '\n' +
            'Скорость: от ' + ctx.scene.state.announcement.speed.split('-')[0] + ' до ' + ctx.scene.state.announcement.speed.split('-')[1] + '\n' +
            'Описание: ' + ctx.scene.state.announcement.description + '\n' +
            'Организаторы: @' + ctx.scene.state.announcement.organizer + 
            (ctx.scene.state.announcement.additionalOrganizers ? ', @' + ctx.scene.state.announcement.additionalOrganizers.join(', @') : '') + '\n\n' +
        ...
        if (text === '✅ Готово') {
            ...
            const validation = announcementService.validateAnnouncement(ctx.scene.state.announcement)
            ...
            const formattedAnnouncementFinal = announcementService.formatAnnouncement(ctx.scene.state.announcement)
            ...
            ctx.scene.state.votingOptions = ctx.scene.state.announcement.votingOptions || []
            
            // Show preview with options to submit or cancel (including voting options for preview)
            const keyboard = Markup.inlineKeyboard([
                [
                    Markup.button.callback('✅ Отправить на модерацию', 'submit_announcement'),
                    Markup.button.callback('❌ Отменить', 'cancel_announcement')
                ]
            ])

            // Add voting options to the preview if they exist
            const votingTextPreview = ctx.scene.state.votingOptions.length > 0
                ? '\n\n🗳 Варианты для голосования:\n' + 
                  ctx.scene.state.votingOptions.map((option, index) => 
                    `${index + 1}. ${option}`
                  ).join('\n')
                : ''
            ...
            const votingOptionsText = ctx.scene.state.announcement.votingOptions.length > 0
                ? '\n\nТекущие варианты для голосования:\n' +
                  ctx.scene.state.announcement.votingOptions.map((option, index) => 
                    `${index + 1}. ${option}`
                  ).join('\n')
                : ''
...
createAnnouncementScene.action('submit_announcement', async (ctx) => {
    ...
    const votingTextModeration = ctx.scene.state.votingOptions.length > 0
        ? '\n\n🗳 Варианты для голосования:\n' + 
          ctx.scene.state.votingOptions.map((option, index) => 
            `${index + 1}. ${option}`
          ).join('\n')
